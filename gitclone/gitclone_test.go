...
var testCases = [...]struct {
	...
	// ** Sparse-checkout **
	{
		...
		wantCmds: []string{
			`git "fetch" "--filter=tree:0" "--no-recurse-submodules"`,
			`git "checkout" "76a934a"`,
		},
	},
	{
		...
		wantCmds: []string{
			`git "fetch" "--filter=tree:0" "--no-recurse-submodules" "origin" "refs/heads/hcnarb"`,
			`git "checkout" "76a934ae"`,
		},
	},
	{
	...
	},
	{
		...
		wantCmds: []string{
			`git "fetch" "--filter=tree:0" "--tags" "--no-recurse-submodules"`,
			`git "checkout" "gat"`,
		},
	},
}
