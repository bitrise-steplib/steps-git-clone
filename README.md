...
| `ignore_branch_for_commit_fetch` | If both commit SHA and branch are available, when checking out a specific commit, the Step normally fetches the branch ref.  This input overrides that default behavior:  - `yes`: Always fetch the commit SHA directly, ignoring any branch information. This requires the Git server to support fetching commits by SHA (uploadpack.allowReachableSHA1InWant). - `no` (default): Use the default behavior (fetch the branch ref if available, otherwise fetch the commit SHA directly). |  | `no` |
